...
/* ─────────────────────────────────────────────
   Category Metadata Helper (icons & photo covers)
───────────────────────────────────────────── */
...
───────────────────────────────────────────── */
const STATIC_JOBS = [
  { name: 'AC Repair', price: 700, area: 'Kothrud', timing: 'Today · 6:00 PM', photos: 2, picked: 3, desc: "AC isn't cooling properly.", img: 'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?w=600&auto=format&fit=crop&q=80', icon: Snowflake },
  { name: 'Home Cleaning', price: 1200, area: 'Baner', timing: 'Tomorrow · 10:00 AM', photos: 4, picked: 2, desc: '2BHK deep clean before moving in.', img: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=600&auto=format&fit=crop&q=80', icon: Sparkles },
  { name: 'Catering', price: 8500, area: 'Wakad', timing: 'Saturday · 4:00 PM', photos: 0, picked: 4, desc: 'Traditional snacks & setup for 40 guests.', img: 'https://images.unsplash.com/photo-1555244162-803834f70033?w=600&auto=format&fit=crop&q=80', icon: Utensils },
  { name: 'Electrical', price: 600, area: 'Hadapsar', timing: 'Today · 3:00 PM', photos: 1, picked: 2, desc: 'Install 3 ceiling fans.', img: 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=600&auto=format&fit=crop&q=80', icon: Zap },
];

const STATIC_PRO_ROLES = ['Electricians', 'Plumbers', 'Cleaners', 'Caterers', 'Photographers', 'Carpenters', 'AC Technicians', 'Painters'];
...
/* Trades photo strip — fallback list if API is offline */
const STATIC_TRADES = [
...
export default function Home() {
      ...
      .then((res) => {
        if (res.data && res.data.length > 0) {
          ...
          setJobs(res.data.map((cat, idx) => {
            const name = cat.catName || cat.cat_name;
            ...
            return {
              ...
              photos: (idx % 3) + 1,
              picked: (idx % 3) + 1,
              desc: cat.description || `${name} services in Pune. Fixed pricing with verified local workers.`,
              img: meta.img,
              icon: meta.icon,
            };
          }));
  ...
  const handleBookCategory = (catName) => {
    const token = localStorage.getItem('token');
  ...
  const displayTrades = categories.length > 0
  ...
  const proRoles = categories.length > 0
    ? categories.map(c => c.catName || c.cat_name)
    : STATIC_PRO_ROLES;
        ...
        <section className="w-full py-14 px-6 border-b" style={{ borderColor: t.border, background: t.bgAlt }}>
          <div className="max-w-7xl mx-auto space-y-6">
            <motion.div {...revealProps} className="flex items-end justify-between flex-wrap gap-3">
              ...
              <span className="wd-mono text-[10px] uppercase tracking-widest" style={{ color: t.faint }}>
                {categories.length > 0 ? `${categories.length} verified categories` : '12+ services & counting'}
              </span>
            </motion.div>
              ...
              {displayTrades.map((tr) => {
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => handleBookCategory(tr.label)}
                    className="group relative aspect-[4/5] rounded-[6px] overflow-hidden cursor-pointer"
                    style={{ background: t.cardHover, border: `1px solid ${t.border}` }}
                  >
              ...
              {featuredJob && (
                ...
                >
                  <div className="relative h-48 overflow-hidden" style={{ background: t.cardHover }}>
                    <img
                      src={featuredJob.img || IMAGES.job}
                      alt=""
                      aria-hidden
                      loading="lazy"
                      className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                  ...
                  <div className="p-7 flex flex-col justify-between gap-8 flex-1">
                    ...
                    <div className="flex items-end justify-between pt-4 border-t" style={{ borderColor: t.border }}>
                      ...
                      <CTA variant="solid" onClick={() => handleBookCategory(featuredJob.name)} t={t}>
                ...
                {secondaryJobs.map((job, idx) => (
                  ...
                  >
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <button
                        onClick={() => handleBookCategory(job.name)}
                        className="wd-mono text-[10px] mt-1 cursor-pointer hover:opacity-70 font-bold"
            ...
            <motion.div {...staggerProps} className="lg:col-span-7 space-y-6">
              ...
              <motion.div variants={childVariants} className="flex flex-wrap items-center gap-3 pt-1">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {proRoles.length > 4 && (
                    <span className="wd-mono text-[10px] px-2.5 py-1 rounded-[4px]" style={{ color: t.faint }}>
                      +{proRoles.length - 4} more
                    </span>
                  )}
                </div>
              </motion.div>
