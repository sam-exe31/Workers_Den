...
export function AudienceSections() {
  ...
  const [workerTypes, setWorkerTypes] = useState(FALLBACK_TYPES);

  useEffect(() => {
    api.get('/Categories')
      .then(res => {
        if (res.data && res.data.length > 0) {
          setWorkerTypes(res.data.map(c => c.catName || c.cat_name));
        }
      })
      .catch(() => { });
  }, []);
  ...
  const handleFindJobs = () => {
    const token = localStorage.getItem('token');
    const user = JSON.parse(localStorage.getItem('user') || 'null');
    if (token && user?.role?.includes('WORKER')) {
      navigate('/worker/find-jobs');
    } else {
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <button
              ...
              className="bg-canvas-dark hover:bg-[#27272A] text-canvas-light font-body font-semibold text-sm px-6 py-3 rounded-btn transition-colors cursor-pointer"
            >
              Post a Job →
            </button>
          </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <button
              ...
              className="bg-accent hover:bg-accent-hover text-canvas-dark font-body font-semibold text-sm px-6 py-3 rounded-btn transition-colors cursor-pointer"
            >
              Find Jobs in Pune →
            </button>
          </div>
